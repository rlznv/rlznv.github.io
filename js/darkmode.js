...
// on first load, if first visit and light mode on, light mode stays on. on first load, if not first visit and dark mode on, dark mode on and will stay on.
// dark mode query function only runs on first visit and if dark mode query on, so can manually switch to light mode after cookie is made and stay in light mode. will NOT work with vscode live preview.
